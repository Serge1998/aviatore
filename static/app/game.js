...
let betAmount = 0;
let balance = parseFloat(window.gameData.balance);
...
let history = window.gameData.history.map(h => parseFloat(h));
...
// Vérifier si le canvas est disponible avant de démarrer
if (canvas && ctx) {
    gameLoop();
} else {
